...
public protocol Routable {
    ...
    /// This will build the api request
    func urlRequestPublisher(enviroment: Enviroment) -> PassthroughSubject<URLRequest, SessionError>
}
...
extension Routable {
    ...
    /// - Returns: URLRequest
    func urlRequestPublisher(enviroment: Enviroment) -> PassthroughSubject<URLRequest, SessionError> {
        let urlRequstPublisher = PassthroughSubject<URLRequest, SessionError>()
        /// Create the url from the base url & url endpoint first
        guard let url = URL(string: (enviroment.baseUrl + self.endPoint)) else {
            urlRequstPublisher.send(completion: .failure(.invalidRequest))
            return urlRequstPublisher
        }
        var urlRequest = URLRequest.init(url: url)
        urlRequest.httpMethod = self.method.rawValue
        /// Merging all headers
